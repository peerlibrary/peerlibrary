  ...
  forward: (db, collectionName, currentSchema, newSchema, callback) =>
    db.collection collectionName, (error, collection) =>
      ...
      collection.update {_schema: currentSchema, maintainerPersons: {$exists: false}, maintainerGroups: {$exists: false}, adminGroups: {$exists: false}}, {$set: {maintainerPersons: [], maintainerGroups: [], adminGroups: []}}, {multi: true}, (error, count) =>
        ...
        cursor = collection.find {_schema: currentSchema, adminPersons: {$exists: false}}, {author: 1}
        document = null
        async.doWhilst (callback) =>
          cursor.nextObject (error, doc) =>
            ...
            collection.update {_schema: currentSchema, _id: document._id, adminPersons: {$exists: false}}, {$set: {adminPersons: [_id: document.author._id]}}, (error, count) =>
              return callback error if error
              callback null
        ,
          =>
