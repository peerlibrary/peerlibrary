class Migration extends Document.MinorMigration
  ...
  forward: (db, collectionName, currentSchema, newSchema, callback) =>
    db.collection collectionName, (error, collection) =>
      ...
      collection.update {_schema: currentSchema, access: {$exists: false}, readPersons: {$exists: false}, readGroups: {$exists: false}}, {$set: {access: Publication.ACCESS.OPEN, readPersons: [], readGroups: []}}, {multi: true}, (error, count) =>
        return callback error if error
