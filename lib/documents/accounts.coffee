...
class @User extends BaseDocument
  ...
  @validateUsername = (username, argumentName) ->
    throw new ValidationError 400, "Username must be at least 3 characters long.", argumentName unless username and username.length >= 3

    throw new ValidationError 400, "Username must contain only a-zA-Z0-9_- characters.", argumentName unless USERNAME_REGEX.test username

    throw new ValidationError 400, "Username already exists.", argumentName if FORBIDDEN_USERNAME_REGEX.test username
    ...
    throw new ValidationError 400, "Username already exists.", argumentName if User.documents.exists username: new RegExp "^#{ username }$", 'i'
    ...
    throw new ValidationError 400, "Username already exists.", argumentName if Person.documents.exists _id: username

  @validatePassword = (password, argumentName) ->
    throw new ValidationError 400, "Password must be at least 6 characters long.", argumentName unless password and password.length >= 6
